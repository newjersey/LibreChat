...
describe('Local files', () => {
  ...
  test('saveLocalBuffer() path w/ `basePath` parameter', async () => {
    ...
    expect(filePath).toEqual(path.posix.join('/', basePath, userId, fileName));

    const expectedPath = path.posix.join(paths.uploads, basePath, userId, fileName);
    expect(fs.existsSync(expectedPath)).toEqual(true);
  });
  ...
  test('saveLocalBuffer() path w/ `basePath` & `temporary` parameters', async () => {
    ...
    expect(filePath).toEqual(path.posix.join('/', basePath, 'tmp', userId, fileName));

    const expectedPath = path.posix.join(paths.uploads, basePath, 'tmp', userId, fileName);
    expect(fs.existsSync(expectedPath)).toEqual(true);
  });
