...
export class KitchenSinkStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: KitchenSinkStackProps) {
    ...
    ollamaService.connections.allowFrom(
      librechatService,
      ec2.Port.tcp(11434),
      'LibreChat to Ollama',
    );
    ...
    librechatService.connections.allowFrom(
      ec2.Peer.ipv4(vpc.vpcCidrBlock),
      ec2.Port.tcp(3080),
      'Health checks to LibreChat',
    );
    librechatService.connections.allowFrom(
      adminPanelService,
      ec2.Port.tcp(3080),
  ...
  private createVectorDbService(
    ...
    const container = taskDef.addContainer('vectordb', {
      image: ecs.ContainerImage.fromRegistry(
        `${this.account}.dkr.ecr.${this.region}.amazonaws.com/newjersey/pgvector:0.8.0-pg15-trixie`,
      ),
  ...
  private createMeiliSearchService(
    ...
    const container = taskDef.addContainer('meilisearch', {
      image: ecs.ContainerImage.fromRegistry(
        `${this.account}.dkr.ecr.${this.region}.amazonaws.com/newjersey/meilisearch:v1.35.1`,
      ),
  ...
  private createOllamaService(
    ...
    const container = taskDef.addContainer('ollama', {
      image: ecs.ContainerImage.fromRegistry(
        `${this.account}.dkr.ecr.${this.region}.amazonaws.com/newjersey/ollama:latest`,
      ),
      logging: ecs.LogDrivers.awsLogs({ streamPrefix: 'lc-ollama' }),
      portMappings: [{ containerPort: 11434 }],
    });
    container.addMountPoints({
      sourceVolume: 'ollamaData',
  ...
  private createRagApiService(
    ...
    taskDef.addContainer('rag_api', {
      image: ecs.ContainerImage.fromRegistry(
        `${this.account}.dkr.ecr.${this.region}.amazonaws.com/newjersey/rag-api:latest`,
      ),
  ...
  private createLibrechatService(
    ...
    taskDef.addContainer('librechat', {
      image: ecs.ContainerImage.fromRegistry(
        `${this.account}.dkr.ecr.${this.region}.amazonaws.com/newjersey/librechat-dev:latest`,
      ),
  ...
  private createAdminPanelService(
    ...
    taskDef.addContainer('librechat-admin-panel', {
      image: ecs.ContainerImage.fromRegistry(
        '152320432929.dkr.ecr.us-east-1.amazonaws.com/newjersey/librechat-admin-panel',
      ),
